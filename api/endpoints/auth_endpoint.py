...
@router.post("/auth/generate_user_id", response_model=UserIdResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(api_key_auth)])
...
async def generate_user_id_endpoint(
    ...
    return auth_service.generate_user_id(role=request.role)

@router.post("/auth/generate_token", response_model=TokenResponse, dependencies=[Depends(api_key_auth)])
@handle_exceptions(tag="[AUTH]")

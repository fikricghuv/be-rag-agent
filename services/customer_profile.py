...
class CustomerProfileService:
    ...
    def get_customer_by_id(self, customer_id: str) -> Optional[Customer]:
        try:
            logger.info(f"[SERVICE][CUSTOMER] Fetching customer by ID: {customer_id}")
    ...
    def get_all_customers(self, limit: int = 10, offset: int = 0, search: Optional[str] = None) -> Dict:
        try:
            logger.info(f"[SERVICE][CUSTOMER] Fetching all customers: offset={offset}, limit={limit}, search='{search}'")
            ...
            logger.info(f"[SERVICE][CUSTOMER] Found {len(data)} customer(s).")
    ...
    def create_customer(self, payload: CustomerCreate) -> Customer:
        try:
            logger.info(f"[SERVICE][CUSTOMER] Creating new customer: {payload.email}")
    ...
    def update_customer(self, customer_id: str, payload: CustomerUpdate) -> Optional[Customer]:
        try:
            logger.info(f"[SERVICE][CUSTOMER] Updating customer: {customer_id}")
            customer = self.get_customer_by_id(customer_id)
            if not customer:
                logger.warning(f"[SERVICE][CUSTOMER] Customer not found: {customer_id}")
                return None
    ...
    def delete_customer(self, customer_id: str) -> bool:
        try:
            logger.info(f"[SERVICE][CUSTOMER] Deleting customer: {customer_id}")
            customer = self.get_customer_by_id(customer_id)
            if not customer:
                logger.warning(f"[SERVICE][CUSTOMER] Customer not found for delete: {customer_id}")
                return False

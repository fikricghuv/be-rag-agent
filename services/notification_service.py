...
class NotificationService:
    ...
    async def create_notification(self, receiver_id: UUID | None, message: str, notif_type: str = "chat", is_broadcast: bool = False):
    ...
    async def get_notifications(self, receiver_id: UUID, limit: int = 20, offset: int = 0):
        logger.info(f"[NOTIF][FETCH] Fetching notifications for user_id={receiver_id}, limit={limit}, offset={offset}")

        query = (
            select(Notification)
            .where(and_(Notification.receiver_id == receiver_id, Notification.is_active == True))
        ...
        count_query = select(func.count()).select_from(
            select(Notification).where(
                and_(Notification.receiver_id == receiver_id, Notification.is_active == True)
            ).subquery()
        )
    ...
    async def mark_notification_as_read(self, notification_id: UUID, receiver_id: UUID):
    ...
    async def soft_delete_all_by_receiver(self, receiver_id: UUID):
